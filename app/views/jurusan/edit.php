...
<div class="max-w-4xl mx-auto bg-white shadow-lg rounded-xl p-8">
    <h3 class="text-xl font-bold text-gray-800 mb-6">Edit Jurusan</h3>

    <!-- Form -->
    <form method="POST" action="/jurusan/<?= $id ?>" class="space-y-5">
            ...
            <select id="gd_id" name="gd_id"
                ...
                <?php foreach ($garduDistribusi as $gd): ?>
                    <option value="<?= $gd['gd_id'] ?>"
                        <?= ($jurusan['gd_id'] ?? '') == $gd['gd_id'] ? 'selected' : '' ?>>
            ...
            <select id="trafo_gardu_id" name="trafo_gardu_id"
                ...
                <?php foreach ($trafoGardu as $tg): ?>
                    <option value="<?= $tg['trafo_gardu_id'] ?>"
                        <?= ($jurusan['trafo_gardu_id'] ?? '') == $tg['trafo_gardu_id'] ? 'selected' : '' ?>>
                        <?= htmlspecialchars($tg['nama_trafo'] ?? 'Trafo ' . $tg['trafo_gardu_id'], ENT_QUOTES, 'UTF-8') ?>
                    </option>
                <?php endforeach; ?>
            </select>
            <?php if (!empty($errors['trafo_gardu_id'])): ?>
                <p class="mt-1 text-sm text-red-600"><?= htmlspecialchars($errors['trafo_gardu_id'][0], ENT_QUOTES, 'UTF-8') ?></p>
            <?php endif; ?>
        </div>
        ...
        <div>
            ...
            <input type="text" id="nama_jurusan" name="nama_jurusan"
                value="<?= htmlspecialchars($jurusan['nama_jurusan'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
            ...
            <?php if (!empty($errors['nama_jurusan'])): ?>
                <p class="mt-1 text-sm text-red-600"><?= htmlspecialchars($errors['nama_jurusan'][0], ENT_QUOTES, 'UTF-8') ?></p>
            <?php endif; ?>
        </div>
        ...
        <div>
            ...
            <textarea id="keterangan" name="keterangan"
                class="mt-1 block w-full rounded-lg border <?= !empty($errors['keterangan']) ? 'border-red-500' : 'border-gray-400' ?> 
                       bg-white text-gray-900 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 text-sm px-3 py-2"><?= htmlspecialchars($jurusan['keterangan'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
