...
<div class="p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        <h2 class="text-2xl font-bold text-gray-800">Master Gardu Hubung</h2>
        <div class="flex items-center gap-3">
            <!-- Search -->
            <form method="GET" action="/gardu-hubung" class="relative">
                <input type="text" name="search" placeholder="Cari Gardu Hubung..." value="<?= htmlspecialchars($search ?? '', ENT_QUOTES, 'UTF-8') ?>"
            ...
            <?php if (!empty($search)): ?>
                <a href="/gardu-hubung" class="text-gray-400 hover:text-gray-600 transition-colors" title="Hapus Pencarian">
    ...
    <div class="overflow-hidden rounded-xl shadow bg-white">
        <table class="min-w-full">
            <thead class="bg-gray-50 border-b">
                <tr>
                    <th class="px-6 py-3 text-left text-sm font-semibold text-gray-700">Kode GH</th>
                    <th class="px-6 py-3 text-left text-sm font-semibold text-gray-700">Nama GH</th>
                    <th class="px-6 py-3 text-left text-sm font-semibold text-gray-700">Alamat</th>
                    <th class="px-6 py-3 text-center text-sm font-semibold text-gray-700">Lat</th>
                    <th class="px-6 py-3 text-center text-sm font-semibold text-gray-700">Lon</th>
                ...
                <?php if (!empty($garduHubungs)): ?>
                    <?php foreach ($garduHubungs as $gh): ?>
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-3 text-sm text-gray-800"><?= htmlspecialchars($gh['kode_gh'] ?? '', ENT_QUOTES, 'UTF-8') ?></td>
                            <td class="px-6 py-3 text-sm text-gray-800 font-medium"><?= htmlspecialchars($gh['nama_gh'] ?? '', ENT_QUOTES, 'UTF-8') ?></td>
                            <td class="px-6 py-3 text-sm text-gray-600"><?= htmlspecialchars($gh['alamat'] ?? '', ENT_QUOTES, 'UTF-8') ?></td>
                            <td class="px-6 py-3 text-sm text-center text-gray-600">
                                <?= htmlspecialchars((string)($gh['lat'] ?? ''), ENT_QUOTES, 'UTF-8') ?>
                            </td>
                            <td class="px-6 py-3 text-sm text-center text-gray-600">
                                <?= htmlspecialchars((string)($gh['lon'] ?? ''), ENT_QUOTES, 'UTF-8') ?>
                            </td>
                            <td class="px-6 py-3 text-center text-sm">
                                <a href="/gardu-hubung/<?= $gh['gh_id'] ?>/edit"
                                    class="text-blue-600 hover:text-blue-800 mx-2 inline-flex items-center">
                                    ✏️ Edit
                                </a>
                                <form method="POST" action="/gardu-hubung/<?= $gh['gh_id'] ?>" class="inline">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="6" class="px-6 py-6 text-center text-gray-500 text-sm">
                            <?php if (!empty($search)): ?>
                                Tidak ada gardu hubung yang sesuai dengan pencarian "<strong><?= htmlspecialchars($search, ENT_QUOTES, 'UTF-8') ?></strong>".
                                <br>
                                <a href="/gardu-hubung" class="text-blue-600 hover:text-blue-800 underline mt-2 inline-block">
                                    Lihat semua data
                                </a>
                            <?php else: ?>
                                Belum ada gardu hubung.
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endif; ?>
...
function buildPaginationUrl($page) {
    $params = $_GET;
    $params['page'] = $page;
    return '/gardu-hubung?' . http_build_query($params);
}
?>
<?php endBlock() ?>
