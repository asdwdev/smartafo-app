...
<div class="max-w-4xl mx-auto bg-white shadow-lg rounded-xl p-8">
    <h3 class="text-xl font-bold text-gray-800 mb-6">Edit Trafo Gardu</h3>

    <!-- Form -->
    <form method="POST" action="/trafo-gardu/<?= $id ?>" class="space-y-5">
        ...
        <div>
            ...
            <?= !empty($errors['gd_id']) ? 'border-red-500' : 'border-gray-400' ?> 
            ...
            focus:border-blue-500 focus:ring-2 focus:ring-blue-500 text-sm px-3 py-2">
                ...
                <?php foreach ($garduDistribusis as $gd): ?>
                    <option value="<?= $gd['gd_id'] ?>"
                        <?= ($trafoGardu['gd_id'] ?? '') == $gd['gd_id'] ? 'selected' : '' ?>>
                        <?= htmlspecialchars($gd['nama_gardu'], ENT_QUOTES, 'UTF-8') ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <div>
            ...
            focus:border-blue-500 focus:ring-2 focus:ring-blue-500 text-sm px-3 py-2">
                ...
                <?php foreach ($trafos as $t): ?>
                    <option value="<?= $t['trafo_id'] ?>"
                        <?= ($trafoGardu['trafo_id'] ?? '') == $t['trafo_id'] ? 'selected' : '' ?>>
            ...
            <?php if (!empty($errors['trafo_id'])): ?>
                <p class="mt-1 text-sm text-red-600">
                    <?= htmlspecialchars($errors['trafo_id'][0], ENT_QUOTES, 'UTF-8') ?>
                </p>
            <?php endif; ?>
        </div>
        ...
        <div>
            ...
            <input type="date" id="tgl_pasang" name="tgl_pasang"
                value="<?= htmlspecialchars($trafoGardu['tgl_pasang'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
            ...
            <?php if (!empty($errors['tgl_pasang'])): ?>
                <p class="mt-1 text-sm text-red-600">
                    <?= htmlspecialchars($errors['tgl_pasang'][0], ENT_QUOTES, 'UTF-8') ?>
                </p>
            <?php endif; ?>
        </div>
        ...
        <div>
            ...
            <input type="date" id="tgl_operasi" name="tgl_operasi"
                value="<?= htmlspecialchars($trafoGardu['tgl_operasi'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
            ...
            <?php if (!empty($errors['tgl_operasi'])): ?>
                <p class="mt-1 text-sm text-red-600">
                    <?= htmlspecialchars($errors['tgl_operasi'][0], ENT_QUOTES, 'UTF-8') ?>
                </p>
            <?php endif; ?>
        </div>
        ...
        <div>
            ...
            <input type="text" id="status_operasi" name="status_operasi"
                value="<?= htmlspecialchars($trafoGardu['status_operasi'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
            ...
            <?php if (!empty($errors['status_operasi'])): ?>
                <p class="mt-1 text-sm text-red-600">
                    <?= htmlspecialchars($errors['status_operasi'][0], ENT_QUOTES, 'UTF-8') ?>
                </p>
            <?php endif; ?>
        </div>
        ...
        <div>
            ...
            <div class="flex gap-6">
                ...
                foreach ($options as $option):
                    $checked = ($trafoGardu['kondisi_fisik'] ?? '') === $option ? 'checked' : '';
                ?>
            ...
            <?php if (!empty($errors['kondisi_fisik'])): ?>
                <p class="mt-1 text-sm text-red-600">
                    <?= htmlspecialchars($errors['kondisi_fisik'][0], ENT_QUOTES, 'UTF-8') ?>
                </p>
            <?php endif; ?>
        </div>
        ...
        <div>
            ...
            <input type="text" id="posisi_arde" name="posisi_arde"
                value="<?= htmlspecialchars($trafoGardu['posisi_arde'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
            ...
            <?php if (!empty($errors['posisi_arde'])): ?>
                <p class="mt-1 text-sm text-red-600">
                    <?= htmlspecialchars($errors['posisi_arde'][0], ENT_QUOTES, 'UTF-8') ?>
                </p>
            <?php endif; ?>
        </div>
        ...
        <div>
            ...
            <input type="text" id="arah_fasa" name="arah_fasa"
                value="<?= htmlspecialchars($trafoGardu['arah_fasa'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
            ...
            <?php if (!empty($errors['arah_fasa'])): ?>
                <p class="mt-1 text-sm text-red-600">
                    <?= htmlspecialchars($errors['arah_fasa'][0], ENT_QUOTES, 'UTF-8') ?>
                </p>
            <?php endif; ?>
        </div>
        ...
        <div>
            ...
            <textarea id="keterangan" name="keterangan"
                class="mt-1 block w-full rounded-lg border <?= !empty($errors['keterangan']) ? 'border-red-500' : 'border-gray-400' ?> 
                       bg-white text-gray-900 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 text-sm px-3 py-2"><?= htmlspecialchars($trafoGardu['keterangan'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
