...
<div class="p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        <h2 class="text-2xl font-bold text-gray-800">Master User Account</h2>
        ...
        <table class="min-w-full">
            <thead class="bg-gray-50 border-b">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-sm font-semibold text-gray-700">Email PLN</th>
                ...
                <?php if (!empty($users)): ?>
                    <?php foreach ($users as $user): ?>
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-3 text-sm text-gray-800"><?= htmlspecialchars($user['nip']) ?></td>
                            <td class="px-6 py-3 text-sm text-gray-800"><?= htmlspecialchars($user['full_name']) ?></td>
                            <td class="px-6 py-3 text-sm text-gray-800"><?= htmlspecialchars($user['email_pln']) ?></td>
                            <td class="px-6 py-3 text-sm text-gray-800"><?= htmlspecialchars($user['area']) ?></td>
                            <td class="px-6 py-3 text-sm text-gray-800"><?= htmlspecialchars($user['level_user']) ?></td>
                            <td class="px-6 py-3 text-sm">
                                <?php if ($user['is_approved'] === 'TRUE'): ?>
                            ...
                            <td class="px-6 py-3 text-center text-sm">
                                <a href="/user-account/<?= $user['user_id'] ?>/edit"
                                    class="text-blue-600 hover:text-blue-800 mx-2 inline-flex items-center">
                                    ✏️ Edit
                                </a>
                                <form method="POST" action="/user-account/<?= $user['user_id'] ?>" class="inline">
