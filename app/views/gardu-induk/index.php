...
<div class="p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        ...
        <div class="flex items-center gap-3">
            <!-- Search -->
            <form method="GET" action="/gardu-induk" class="relative">
                <input type="text" name="search" placeholder="Cari Gardu Induk..." value="<?= htmlspecialchars($search ?? '', ENT_QUOTES, 'UTF-8') ?>"
            ...
            <?php if (!empty($search)): ?>
                <a href="/gardu-induk" class="text-gray-400 hover:text-gray-600 transition-colors" title="Hapus Pencarian">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="6" class="px-6 py-6 text-center text-gray-500 text-sm">
                            <?php if (!empty($search)): ?>
                                Tidak ada gardu induk yang sesuai dengan pencarian "<strong><?= htmlspecialchars($search, ENT_QUOTES, 'UTF-8') ?></strong>".
                                <br>
                                <a href="/gardu-induk" class="text-blue-600 hover:text-blue-800 underline mt-2 inline-block">
                                    Lihat semua data
                                </a>
                            <?php else: ?>
...
function buildPaginationUrl($page) {
    $params = $_GET;
    $params['page'] = $page;
    return '/gardu-induk?' . http_build_query($params);
}
?>
<?php endBlock() ?>
