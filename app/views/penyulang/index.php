...
<div class="p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        ...
        <div class="flex items-center gap-3">
            <!-- Search -->
            <form method="GET" action="/penyulang" class="relative">
                <input type="text" name="search" placeholder="Cari Penyulang..." value="<?= htmlspecialchars($search ?? '', ENT_QUOTES, 'UTF-8') ?>"
            ...
            <?php if (!empty($search)): ?>
                <a href="/penyulang" class="text-gray-400 hover:text-gray-600 transition-colors" title="Hapus Pencarian">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-3 text-sm">
                                <span class="px-2 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-700">
                                    <?= htmlspecialchars($p['area'] ?? '', ENT_QUOTES, 'UTF-8') ?>
                                </span>
                            </td>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="6" class="px-6 py-6 text-center text-gray-500 text-sm">
                            <?php if (!empty($search)): ?>
                                Tidak ada penyulang yang sesuai dengan pencarian "<strong><?= htmlspecialchars($search, ENT_QUOTES, 'UTF-8') ?></strong>".
                                <br>
                                <a href="/penyulang" class="text-blue-600 hover:text-blue-800 underline mt-2 inline-block">
                                    Lihat semua data
                                </a>
                            <?php else: ?>
                                Belum ada penyulang.
...
function buildPaginationUrl($page) {
    $params = $_GET;
    $params['page'] = $page;
    return '/penyulang?' . http_build_query($params);
}
?>
<?php endBlock() ?>
