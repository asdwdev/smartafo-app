...
class GarduDistribusi extends Model
{
    protected $table = "gardu_distribusi";
    ...
    public function paginate($page = 1, $perPage = 10, $search = '')
    {
        ...
        if (!empty($search)) {
            $searchCondition = " WHERE (kode_gardu LIKE :search1 OR nama_gardu LIKE :search2 OR alamat LIKE :search3 OR keterangan LIKE :search4)";
            $searchTerm = "%{$search}%";
            $params = [
                'search1' => $searchTerm,
                'search2' => $searchTerm,
    ...
    /**
     * Search gardu distribusi by multiple fields
     */
