...
class GarduHubung extends Model
{
    protected $table = "gardu_hubung";
    ...
    public function paginate($page = 1, $perPage = 10, $search = '')
    {
        ...
        if (!empty($search)) {
            $searchCondition = " WHERE (kode_gh LIKE :search1 OR nama_gh LIKE :search2 OR alamat LIKE :search3 OR CAST(lat AS TEXT) LIKE :search4 OR CAST(lon AS TEXT) LIKE :search5)";
            $searchTerm = "%{$search}%";
            $params = [
                'search1' => $searchTerm,
                'search2' => $searchTerm,
    ...
    /**
     * Search gardu hubung by multiple fields
     */
